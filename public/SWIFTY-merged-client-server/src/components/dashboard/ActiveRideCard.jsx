...
export function ActiveRideCard() {
  const ride = useStore((s) => s.deliveries.find((item) => item.type === 'ride' && !['delivered', 'cancelled'].includes(item.status)))
  if (!ride) return null
  ...
  return <section className="overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-slate-200/70">
    <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 sm:px-6">
      ...
      <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700">{ride.etaMinutes || 0} min</span>
    </div>
    <div className="grid lg:grid-cols-[1fr_280px]">
      ...
      <div className="space-y-5 p-5 sm:p-6">
        <div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-full bg-slate-100"><CarFront className="h-5 w-5" /></div><div><p className="text-sm font-bold">{ride.riderName || rider.name || 'Swifty rider'}</p><p className="text-xs text-slate-500">{rider.vehicleType || 'Driver assigned'}{rider.plateNumber ? ` • ${rider.plateNumber}` : ''}</p></div></div>
        <div className="space-y-3 text-sm"><div className="flex gap-3"><MapPin className="mt-0.5 h-4 w-4 text-emerald-600" /><span>{ride.pickup?.address}</span></div><div className="ml-2 h-5 border-l border-dashed border-slate-300" /><div className="flex gap-3"><MapPin className="mt-0.5 h-4 w-4 text-slate-900" /><span>{ride.dropoff?.address}</span></div></div>
