...
import { assignAvailableRider, createRide, updateDeliveryStatus, useStore } from '../../lib/api-store'
...
export default function Ride() {
  ...
  useEffect(() => {
    ...
    const findRider = async () => {
      if (cancelled) return
      try {
        const matched = await assignAvailableRider(searchingRideId)
        if (matched?.riderId || matched?.rider) {
  ...
  async function confirmRide() {
    ...
    try {
      const order = await createRide({
        pickup: { address: pickup || 'My location', coords: pickupCoords || LAGOS },
  ...
  function cancelRiderSearch() {
    ...
    if (rideId) void updateDeliveryStatus(rideId, 'cancelled')
    setSearchingRideId(null)
