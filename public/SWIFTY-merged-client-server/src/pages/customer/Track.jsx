...
import { useStore, updateDeliveryStatus, STATUS_LABEL, getCurrentUser } from '@/lib/api-store'
...
export default function Track() {
  ...
  useEffect(() => {
    ...
    let active = true
    getPublicTracking(id).then((data) => { if (active) setPublicDelivery(data.tracking) }).catch(() => {})
    return () => { active = false }
  }, [id, delivery])
...
const handleSheetHeightChange = useCallback((px, dragging = true) => {
  ...
  function handleCancel() {
    updateDeliveryStatus(deliveryView.id, 'cancelled')
  }
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className={`mt-1 font-display text-2xl font-bold ${isCancelled ? 'text-red-500' : isDelivered ? 'text-emerald-600' : 'text-emerald-600'}`}>
                {STATUS_LABEL[deliveryView.status]}
              </p>
            </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-3 flex items-center gap-4">
            ...
            <div className="flex-1">
              <p className="text-sm font-bold">{deliveryView.riderName ?? 'Awaiting rider…'}</p>
              {riderAssigned && (
