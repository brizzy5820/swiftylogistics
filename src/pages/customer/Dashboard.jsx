...
import { ArrowRight, Clock, PackageCheck, Search, ClipboardClock, Loader } from "lucide-react";
...
const PLAN_CARDS = [
  { title: "Book a ride", subtitle: "A car is minutes away, day or night", image: "/formimg.png" },
  {
    ...
    subtitle: "Package pickup and drop-off in minutes",
    image: "/deliveryguy.png",
  },
  {
    title: "Track an order",
    subtitle: "Follow it live, every step of the way",
    image: "/plan-track.png",
  },
];
...
export default function CustomerDashboard() {
  ...
  if (!user) return null;
  const firstName = accountUser.name
              ...
              <div className="flex flex-col justify-between gap-8 sm:flex-row sm:items-end">
                <div>
                  <h1 className="mt-2 max-w-xl font-display text-3xl font-black tracking-tight sm:text-4xl">
                    Good to see you, {`${firstName.split(' ')[1]}`}!
                  </h1>
                  <p className="mt-3 max-w-xl text-sm leading-6 text-slate-200">
                    Ride across town, send a package, or track something already on its way.
                  </p>
                </div>
              </div>
              <div className="mt-8 flex max-w-xl items-center gap-3 rounded-2xl bg-white/10 p-2 ring-1 ring-white/10">
                <Search className="ml-2 h-4 w-4 text-slate-400" />
                <input
                  placeholder="Search destination, tracking ID or service"
                  className="min-w-0 flex-1 bg-transparent px-1 py-2 text-sm text-white outline-none placeholder:text-slate-300"
                />
                <button className="rounded-xl bg-white px-4 py-2 text-xs font-bold text-slate-900">
                  Search
                </button>
              </div>
            </div>
            ...
            <section>
              <div className="mb-3 flex items-center justify-between">
                <div>
                  ...
                  <h2 className="mt-1 font-display text-xl font-bold">Recent orders</h2>
                </div>
                <Link to="/customer/history" className="text-sm font-bold text-emerald-700">
                  See all
                </Link>
              </div>
                ...
                {deliveriesLoading ? (
                  ...
                  recent.map((item) => (
                    <Link
                      ...
                      className="flex items-center gap-4 border-b border-slate-100 p-4 last:border-0 hover:bg-slate-50"
                    >
            ...
            <div className="divide-y divide-slate-100 overflow-hidden rounded-2xl bg-white ring-1 ring-slate-200/50">
              ...
              {recent.length ? (
                recent.map((item) => (
                  ...
                  >
                    <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gray-100 text-emerald-700">
