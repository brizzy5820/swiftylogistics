...
export default function Index() {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-white text-slate-900">
      <header
        className={`fixed inset-x-0 top-0 z-50 transition ${scrolled ? "border-b border-slate-200/70 bg-white/90 shadow-sm backdrop-blur-xl" : "bg-transparent"}`}
      >
        <div className="mx-auto flex h-15 max-w-7xl items-center justify-between px-5 sm:px-8 lg:px-10">
          <Link
            to="/"
            className={`text-2xl font-black tracking-tight ${scrolled ? "text-slate-950" : "text-slate-900"}`}
          >
            Swifty<span className="text-emerald-500">.</span>
          </Link>
          <nav
            className={`hidden items-center gap-7 text-sm font-semibold md:flex ${scrolled ? "text-slate-600" : "text-slate-700"}`}
          >
          ...
          <Sheet>
            <SheetTrigger asChild>
              <button
                className={`rounded-xl p-2.5 sm:hidden ${scrolled ? "text-slate-900" : "text-slate-900"}`}
                aria-label="Menu"
              >
                <Menu />
              </button>
            ...
            <SheetContent>
              <SheetHeader>
                <SheetTitle>swifty.</SheetTitle>
              </SheetHeader>
              <div className="mt-6 grid gap-2">
                <a href="#services" className="rounded-xl p-3 font-semibold">
                  Services
                </a>
      ...
      <section className="relative overflow-hidden bg-emerald-50/60">
        <motion.div style={{ y: heroY }} className="absolute inset-0 scale-105 opacity-90">
          <MapBackdrop />
        </motion.div>
        ...
        <div className="relative z-10 mx-auto flex min-h-[600px] items-center  pb-12 pt-25 sm:px-8 sm:pb-16  lg:grid lg:grid-cols-[1.05fr_.95fr] lg:min-h-[760px] lg:px-10 lg:pt-32">
          <div className="w-full py-6 ">
            ...
            <motion.h1
              ...
              className="max-w-3xl font-display text-4xl font-black leading-[.98] tracking-[-.04em] text-slate-950 sm:text-6xl px-5 lg:px-none lg:text-7xl"
            >
              Move your world <span className="text-emerald-500">with Swifty.</span>
            </motion.h1>
            <p className="mt-6 max-w-xl text-base leading-7 px-5 lg:px-none text-slate-600 sm:text-lg">
              Book a ride, send a package or follow any trip — all from one calm map. Tell us where,
            ...
            <motion.div
              ...
              className="mt-9 max-w-xl rounded-3xl border border-slate-200/80 bg-white/85 p-4  backdrop-blur-xl"
            >
              <div className="flex rounded-full bg-slate-100 p-1 text-sm font-bold">
                <button
                  type="button"
                  onClick={() => setMode("ride")}
                  className={`flex-1 rounded-full py-2.5 transition ${mode === "ride" ? "bg-emerald-500 text-white shadow" : "text-slate-600"}`}
                >
                  Request a ride
                </button>
                <button
                  ...
                  className={`flex-1 rounded-full py-2.5 transition ${mode === "track" ? "bg-emerald-500 text-white shadow" : "text-slate-600"}`}
                >
                  Track a trip
                </button>
              </div>
              ...
              {mode === "ride" ? (
                <form onSubmit={handleRide} className="mt-4">
                  ...
                  <button
                    type="submit"
                    className="mt-4 flex items-center bg-emerald-500 gap-2 rounded-full px-6 py-2.5 text-white text-sm font-bold shadow-sm transition-all "
                  >
                    Find ride <ArrowRight className="h-4 w-4" />
                  </button>
                </form>
              ) : (
                <form onSubmit={handleTrack} className="mt-4 flex flex-col gap-3 sm:flex-row">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      className="w-full rounded-2xl bg-slate-50 py-3.5 pl-10 pr-4 text-sm font-semibold text-slate-800 outline-none placeholder:text-slate-400 focus:border focus:border-emerald"
                    />
                  </div>
                  <button
                    type="submit"
                    className="flex items-center bg-emerald-500 gap-2 rounded-full px-6 py-2.5 text-white text-sm font-bold shadow-sm transition-all "
                  >
                    Track now
                  </button>
                </form>
      ...
      <section className="border-b border-slate-200 bg-white py-10 sm:py-14">
        <div className="mx-auto grid max-w-7xl grid-cols-2 gap-6 px-6 sm:grid-cols-4 sm:px-8 lg:px-10 sm:gap-8">
          <Metric value="50k+" label="completed deliveries" />
          <Metric value="1.2k+" label="active riders" />
          <Metric value="4.9/5" label="average experience" />
          <Metric value="24/7" label="movement support" />
        </div>
      </section>
      <section id="services" className="bg-slate-50 px-6 py-16 sm:px-8 sm:py-24 lg:px-10">
        <div className="mx-auto max-w-7xl">
          <div className="max-w-2xl">
            ...
            <p className="mt-4 text-base leading-7 text-slate-500">
              Swifty is evolving from a package service into a complete movement platform for
      ...
      <section id="how" className="px-6 py-16 sm:px-8 sm:py-24 lg:px-10">
        <div className="mx-auto grid max-w-7xl gap-10 lg:gap-12 lg:grid-cols-[.8fr_1.2fr] lg:items-center">
          <div>
            <p className="text-sm font-bold uppercase tracking-[.18em] text-emerald-700">
              Simple by design
            </p>
            <h2 className="mt-3 font-display text-3xl font-black tracking-tight sm:text-4xl lg:text-5xl">
              One flow for every journey.
            </h2>
            <p className="mt-5 leading-7 text-slate-500">
              Whether you need a driver or a courier, the experience stays familiar: choose a
              service, set your destination, confirm and follow the progress.
            </p>
          </div>
          <div className="grid gap-4 sm:grid-cols-3">
            <Step
              ...
              icon={Smartphone}
              title="Choose"
              text="Select ride, delivery or another service."
            />
            <Step
              n="02"
              icon={MapPin}
              title="Set the route"
              text="Tell us where to pick up and where to go."
            />
            <Step n="03" icon={Clock3} title="Move" text="Get live updates from start to finish." />
          </div>
        </div>
      </section>
